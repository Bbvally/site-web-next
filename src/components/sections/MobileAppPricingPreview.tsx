...
import { mobileappPreviewData } from "@/data/homepage";
import { CheckCircle, ArrowRight } from "@/components/icons";

export function MobileAppPricingPreview() {
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="font-heading text-3xl font-bold text-text-primary sm:text-4xl">
            {mobileappPreviewData.title}
          </h2>
          <p className="mt-4 text-lg text-text-secondary">
            {mobileappPreviewData.subtitle}
          </p>
        </div>

        {/* Pricing cards */}
        <div className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {mobileappPreviewData.plans.map((plan, index) => (
            <motion.div
              key={plan.name}
        ...
        <div className="mt-8 text-center">
          <Link
            href={mobileappPreviewData.viewAllHref}
